...
function main(config) {
    ...
    if (ruleOptions.openai) {
        ...
        config['proxy-groups'].push({
            ...
            name: '国外AI',
            type: 'select',
            proxies: ['OK', '指定', ...proxyGroupsRegionNames, '直连'],
    ...
    if (ruleOptions.youtube) {
        ...
        config['proxy-groups'].push({
            ...
            name: 'YouTube',
            type: 'select',
            proxies: ['指定', ...proxyGroupsRegionNames, '直连'],
    ...
    if (ruleOptions.biliintl) {
        ...
        config['proxy-groups'].push({
            ...
            name: '哔哩哔哩东南亚',
            type: 'select',
            proxies: ['指定', '直连', ...proxyGroupsRegionNames],
    ...
    config['proxy-groups'].push(
        ...
        },
        {
            ...
            name: '其他外网',
            type: 'select',
            proxies: ['指定', '国内网站', ...proxyGroupsRegionNames],
        ...
        },
        {
            ...
            name: '国内网站',
            type: 'select',
            proxies: ['直连', '指定', ...proxyGroupsRegionNames],
    ...
    // 新增分组：OK（排除含香港关键词的节点）、指定（包含OK并包含所有可用节点但排除直连）
    const hkRegex = /港|🇭🇰|hk|hongkong|剩余|kitty|到期|节点|hong kong/i
    const allNames = config.proxies.map((p) => p.name)
    const okNames = allNames.filter((n) => !hkRegex.test(n))
    const allNonDirectNames = allNames.filter((n) => n !== '直连')
